...
using namespace std;
const int port = 5050;	//监听端口号
//响应首部内容
const char* http_get_message_success = "HTTP/1.1 200 OK \r\n\Content-Length:%d\r\nConnection:close\r\nContent-Type:%s\r\n\r\n";
const char* http_404_error = "HTTP/1.1 404 Not Found \r\n\Content-Length:%d byte(s)\r\nConnection:close\r\nContent-Type:%s\r\n\r\n";
...
int http_send_response(SOCKET soc, char* buf, int buf_len, char* file_name)
{
	...
	if (type == NULL)
	{
		...
		if (!strlen(file_name)) {
			strcpy(file_read_name, "index.html");
		}
		else {
			printf("error!\n");
...
void main() {
	...
	while (true) {
		...
		//创建会话SOCKET成功
		cout << "Socket accept Ok!Connection from IP:";
