...
bool SRRdtSender::send(const Message& message) {
	...
	pUtils->printPacket("hah999a", this->packetWaitingAck[nextseqnum]);
...
void SRRdtSender::receive(const Packet& ackPkt) {
	...
	if (checkSum == ackPkt.checksum) {
		...
		if (rcvpkt[ackPkt.acknum] == false &&(ackPkt.acknum - send_base + SR_SEQ_NUM) % SR_SEQ_NUM < SR_WINDOW_SIZE)
		{
			...
			pUtils->printPacket("hah33a", ackPkt);
...
void SRRdtSender::timeoutHandler(int seqNum) {
	...
	cout << "haha2" << seqNum << "haha";
	pns->stopTimer(SENDER, seqNum);										//首先关闭定时器
	pUtils->printPacket("hahalal", packetWaitingAck[seqNum]);
