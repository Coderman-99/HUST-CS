...
bool TCPRdtSender::send(const Message& message) {
	...
	pUtils->printPacket("anis", this->packetWaitingAck[nextseqnum]);
...
void TCPRdtSender::receive(const Packet& ackPkt) {
	...
	if (checkSum == ackPkt.checksum)
	{
		...
		{	//对已经确认的报文段的一个冗余ACK
			...
			cout << "oknabro" << extraack << "ACK seqnum = " << ackPkt.acknum << endl;
			if (extraack == 3)	//如果收到三个冗余ACK，则执行快速重传
			{
				pUtils->printPacket("olala",this->packetWaitingAck[y]);
				//重新发送具有序号y的报文段
...
void TCPRdtSender::timeoutHandler(int seqNum) {
	//单一的超时计算器
	pUtils->printPacket("ololo", this->packetWaitingAck[sendbase]);
